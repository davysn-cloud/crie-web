...
## Histórico
- **2026-04-29** — Deploy agent: review crítico de infra/deploy/integrações em [[../02_architecture/review-2026-04-29-infra|review-2026-04-29-infra]]. Top riscos: região Supabase não confirmada (us-east vs sa-east-1), zero observabilidade configurada, App Review Meta não submetido (4-8sem). Top ações: migrar/confirmar sa-east-1, setup Sentry+Axiom+BetterStack pré-beta, submeter App Review Meta agora, prompt caching Anthropic, abstração billing antes de Stripe SDK. Custo: ~$155/mês a 100 users, ~$565/mês a 500 users. Bottleneck: Supabase storage egress + LLM (se subsidiarmos).
...
- **2026-04-29** — Prospector agent: processo detalhado de scan de agencias no Instagram criado em [[../06_deploy/prospeccao/instagram-scan-process]]. Cobre 8 etapas (escolha de hashtag, navegacao no feed, avaliacao de perfil, scoring ICP, registro, identificacao do decision maker, canal de abordagem, fechamento de sessao), metricas de eficiencia, erros comuns e cadencia recomendada para o beta (meta: 30-45 leads/mes, 3-5 agencias piloto).
- **2026-04-29** — Frontend reviewer: review de escala/latencia em [[../02_architecture/review-2026-04-29-frontend]]. Riscos top: (1) zero code splitting — 41 paginas em import estatico no App.tsx; (2) `/a/:token` carrega bundle inteiro do app autenticado, caminho critico do aprovador mobile; (3) zero telemetria (Sentry/web-vitals) — voando cego. Top 5 acoes priorizadas: lazy routes + manualChunks, bundle/PWA isolado para aprovador, telemetria, pipeline de imagens responsivas (AVIF/WebP via Edge Function existente), virtualizacao kanban + singleton Realtime por workspace.
- **2026-04-29** — Coordinator: review completo de arquitetura/stack para escala disparado em paralelo (backend + frontend + infra). Sumario executivo em [[../02_architecture/review-2026-04-29-summary]]; detalhes em [[../02_architecture/review-2026-04-29-backend]] e [[../02_architecture/review-2026-04-29-infra]]. Veredito: stack atual (React+Vite+Supabase+Vercel+Stripe+Resend) sustenta beta. 6 acoes P0 pre-beta: confirmar Supabase em sa-east-1, migration de saneamento (indices FK + desnormalizar workspace_id em publish_attempts/approval_pins/carousel_slides para eliminar subquery em RLS), fechar inconsistencia pgsodium vs pgcrypto em agency_integrations, code-splitting/lazy routes, bundle isolado+PWA para `/a/:token`, observabilidade (Sentry+web-vitals+BetterStack). 5 acoes P1 durante beta (particionar audit_log/insights, pipeline de imagens responsivas, App Review Meta, Realtime singleton+virtualizacao, prompt caching+llm_usage). 5 trocas planejadas com gatilhos: Storage→R2+CF Images (>100GB egress), pg_cron→Trigger.dev (>200 publishes/dia), Vite SPA→TanStack Start/Next (LCP p75 mobile >2.5s), Realtime→broadcast/Liveblocks (>150 conexoes pico), +Asaas se Pix dominar. Custo projetado: ~$155/mes a 100 users, ~$565/mes a 500 users; bottleneck em Supabase compute+egress e LLM se subsidiarmos.
